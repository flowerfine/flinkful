...
const executeSql = async (param: IExecuteSqlParams) => {
  return fetch("/data/connection/execute.json").then((response) => response.json()).then(resp => resp.data);
};
...
/** 获取库的所有表 */
const getAllTableList = async (param: { dataSourceId: number; databaseName?: string | null; schemaName?: string | null }) => {
  return fetch("/data/connection/table_list.json").then((response) => response.json()).then(resp => resp.data);
};
